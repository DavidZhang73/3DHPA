...
def tag_tr(k, data, table_title):
    ...
    for d in data:
        ...
        if d_type == "img":
            s += '<td><a href="{}"><img src="{}" width="200px" height="200px" /></a></td>'.format(
                os.path.join("..", d_value), os.path.join("..", d_value)
            )
        elif d_type == "text":
            s += f"<td>{d_value}</td>"
        else:
...
def func(
    ...
    num_per_page,
    input_main_folder,
    input_folder_list_comma,
    input_title_list_comma,
    first_hierachy,
    fout,
    prompt,
):
    ...
    html_head = (
        "<html><head><title>Simple Viewer</title>"
    ...
    if os.path.isdir(os.path.join(input_main_folder, "ids")):
        filelist = [
            int(f.split(".")[0]) for f in os.listdir(os.path.join(input_main_folder, "ids")) if f.endswith(".id")
        ]
        filelist.sort()
        ...
        filelist = [
            f.split(".")[0]
            for f in os.listdir(os.path.join(input_main_folder, input_folder_list[0]))
    ...
    for f in file_name_list:
        if first_hierachy and k % num_per_page == 0:
            ...
            fout.write(f"<h3>Folder: {input_folder_list}, Page Id: {k // num_per_page}</h3>")
            ...
            fout.write(
                '<h3><a href="{}">First</a>&nbsp;&nbsp;&nbsp;<a href="{}"{}>Prev</a>&nbsp;&nbsp;&nbsp;<a href="{}"{}>Next</a>&nbsp;&nbsp;&nbsp;<a href="{}">Last</a></h3>'.format(
                    str(0) + ".html",
                    str(prev_id) + ".html",
                    prev_click,
                    str(next_id) + ".html",
                    next_click,
                    str(int(tot_page_num - 1)) + ".html",
                )
            )
        ...
        for input_folder in input_folder_list:
            if os.path.exists(os.path.join(input_main_folder, input_folder, str(f) + ".jpg")):
                data[counter] = (
                    "img",
                    os.path.join(input_main_folder, input_folder, str(f) + ".jpg"),
                )
            elif os.path.exists(os.path.join(input_main_folder, input_folder, str(f) + ".png")):
                data[counter] = (
                    "img",
                    os.path.join(input_main_folder, input_folder, str(f) + ".png"),
                )
        ...
        if num_hierachy > 1:
            fout.write(
                '<button style="margin: 1em 1em 1em 1em; font-size: 15px; border-radius:7px;">Click Here to See/Hide More Details</button>'
            )
            fout.write('<div class="folder" style="margin: 0 3em 0 3em; ">')
            func(
                output_folder,
                1000,
                os.path.join(input_main_folder, "child", str(f)),
                input_folder_list_others,
                input_title_list_others,
                False,
                fout,
                prompt + "\t",
            )
...
# main
USAGE = (
    "USAGE: \t\tgen_html_hierachy folder num_per_page output_folder_name input_folder_list input_title_list [optional: num_log_lines_to_show, default: 1]\n"
