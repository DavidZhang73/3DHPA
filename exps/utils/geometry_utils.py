...
def load_pts(fn):
    with open(fn) as fin:
        ...
        pts = np.array(
            [[float(line.split()[0]), float(line.split()[1]), float(line.split()[2])] for line in lines],
            dtype=np.float32,
        )
        return pts
...
def load_pts_nor(fn):
    with open(fn) as fin:
        ...
        nor = np.array(
            [[float(line.split()[3]), float(line.split()[4]), float(line.split()[5])] for line in lines],
            dtype=np.float32,
        )
        return pts, nor
...
def sample_points_from_obj(label_fn, obj_fn, pts_fn, num_points, verbose=False):
    cmd = "MeshSample -n%d -s3 -l %s %s %s> /dev/null" % (
        num_points,
        label_fn,
        obj_fn,
        pts_fn,
    ...
    with open(pts_fn) as fin:
        lines = [line.rstrip() for line in fin]
        pts = np.array(
            [[line.split()[0], line.split()[1], line.split()[2]] for line in lines],
            dtype=np.float32,
        )
        label = np.array([int(line.split()[-1].split('"')[1]) for line in lines], dtype=np.int32)
        if verbose:
            print("get pts: ", pts.shape)
...
def export_pts_with_color(out, pc, color):
    num_point = pc.shape[0]
    with open(out, "w") as fout:
        for i in range(num_point):
            fout.write("%f %f %f %d %d %d\n" % (pc[i, 0], pc[i, 1], pc[i, 2], color[i, 0], color[i, 1], color[i, 2]))
...
def render_pts_with_label(out, pts, label, delete_img=False, base=0, point_size=6):
    ...
    cmd = "RenderShape %s -l %s %s 600 600 -p %d > /dev/null" % (
        tmp_pts,
        tmp_label,
        out,
        point_size,
...
def render_pts(out, pts, delete_img=False, point_size=6, point_color="FF0000FF"):
    ...
    cmd = "RenderShape %s %s 600 600 -p %d -c %s > /dev/null" % (
        tmp_pts,
        out,
        point_size,
        point_color,
