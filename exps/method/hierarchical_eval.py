...
def eval_func(val_loader, model, log_writer, args):
    ...
    sum_part_cd_loss = 0.0
    sum_shape_cd_loss = 0.0
    sum_rot_cd_loss = 0.0
    sum_part_cd_loss = 0
    sum_trans_l2_loss = 0.0
    sum_contact_point_loss = 0.0
    total_acc_part = 0.0
    total_valid_part = 0.0
    total_contact_correct = 0.0
    total_contact_point = 0.0
    num_ins = 0
    real_val_data_set = 0
    for i, batch_data in enumerate(val_loader):
        ...
        with torch.no_grad():
            (
                ...
                acc_per_batch,
                valid_per_batch,
                contact_correct_per_batch,
                contact_point_per_batch,
                batch_size,
                output,
                cdsV1_sum,
                cdsV2_sum,
        ...
        part_acc.update((acc_per_batch / valid_per_batch).item() * 100.0, batch_size)

        cdsV1_loss_dist.update(cdsV1_sum.item(), part_pcs.shape[0])
        cdsV2_loss_dist.update(cdsV2_sum.item(), part_pcs.shape[0])

        connectivity_acc.update(
            (contact_correct_per_batch / contact_point_per_batch).item() * 100.0,
            batch_size,
        )
    ...
    res_part_acc = total_acc_part / total_valid_part * 100.0
    res_contact_acc = total_contact_correct / total_contact_point * 100.0
