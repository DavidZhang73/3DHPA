...
from .func import (
    get_contact_point_loss,
    get_contact_point_loss_for_single_part,
    get_rot_cd_loss,
    get_rot_l2_loss,
    get_shape_cd_loss2,
    get_shape_cd_loss_default,
    get_total_cd_loss,
    get_trans_l2_loss,
    linear_assignment,
)
...
def shape_diversity_score(shapes, batch_size):
    ...
    for i in range(len(shapes)):
        for j in range(len(shapes)):
            ...
            cdsV2 += (
                shape_cd_loss_per_data * shapes[i][4] * shapes[j][4] * ca_check_one_fun(shapes[i][4], shapes[j][4], 0.5)
            )
...
def infer_execute(
    ...
    for bs_ind in range(batch_size):
        ...
        for ins_id in range(1, num_part + 1):
            ...
            matched_pred_ids, matched_gt_ids = linear_assignment(
                cur_pts, cur_pred_centers, cur_pred_quats, cur_gt_centers, cur_gt_quats
            )
    ...
    contact_point_loss_per_trans, num_correct_contact, num_contact_points = get_contact_point_loss(
        pred_trans, pred_rot, contact_points, sym_info
    )
    ...
    return (
        ...
        pred_poses_per_trans,
        total_cd_loss_per_trans,
        shape_cd_loss_per_trans,
        contact_point_loss_per_trans,
        num_correct_contact,
        num_contact_points,
    )
...
def inference(
    ...
    acc2, *rest2 = infer_execute(
        pred_poses_per_trans2,
        gt_poses,
        part_pcs,
        part_valids,
        match_ids,
...
def decode_eval(
    ...
    # shape cd loss.
    shape_dist_1, shape_dist_2 = get_shape_cd_loss_default(
        part_pcs, pred_rot, gt_rot, pred_trans, gt_trans, part_valids, return_raw=True
    )
    ...
    contact_point_loss, num_correct_contact, num_contact_points = get_contact_point_loss_for_single_part(
        pred_trans, pred_rot, contact_points, sym_info, part_mask=part_mask
    )
